...
TEST_CASE("Sparse matrix residual test", "[sparse][residual]") {
    ...
    // For a zero-initialized SparseMatrix, b - A*x should equal b
    for (size_t i = 0; i < r.size(); ++i) {
        REQUIRE(r(i) == Approx(b(i)).epsilon(1e-12));
    }
}

TEST_CASE("Sparse matrix Frobenius norm", "[sparse][norm]") {
    using namespace LinearAlgebra;
...
TEST_CASE("Sparse matrix scalar multiplication", "[sparse][scalarmult]") {
    using namespace LinearAlgebra;
...
TEST_CASE("Sparse matrix matrix-vector product")
{
    using namespace LinearAlgebra;
...
TEST_CASE("Sparse matrix matrix-matrix product")
{
    using namespace LinearAlgebra;
    SparseMatrix<double> A(5, 5);
    SparseMatrix<double> B(5, 5);
...
TEST_CASE("Sparse matrix matrix addition")
{
    using namespace LinearAlgebra;
    SparseMatrix<double> A(5, 5);
    SparseMatrix<double> B(5, 5);
...
TEST_CASE("Sparse matrix matrix subtraction")
{
    using namespace LinearAlgebra;
    SparseMatrix<double> A(5, 5);
    SparseMatrix<double> B(5, 5);
...
TEST_CASE("Sparse matrix extract diagonal")
{
    using namespace LinearAlgebra;
    SparseMatrix<double> A(5, 5);
    for (int i = 0; i < A.rows(); i++)
    ...
    SparseMatrix<double> C = A.diagonalMatrix();
...
TEST_CASE("Sparse matrix invert diagonal")
{
    using namespace LinearAlgebra;
    ...
    SparseMatrix<double> C = A.inverseDiagonalMatrix();
...
TEST_CASE("Sparse matrix eliminateRowAndColumn")
{
    using namespace LinearAlgebra;
    SparseMatrix<double> A(4, 4);
